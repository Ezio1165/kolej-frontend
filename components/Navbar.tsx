...
interface NavbarProps {
    logoUrl: string | null;
    siteName: string;
    menuMap: Record<string, any[]>; // Strapi'den gelen menü yapısı
}
...
export default function Navbar({ logoUrl, siteName, menuMap }: NavbarProps) {
    ...
    const [hoveredMenu, setHoveredMenu] = useState<string | null>(null); // Hangi menü açık?
    const pathname = usePathname();

    // Pathname undefined gelirse varsayılan olarak "/" kabul etme
    ...
    const loginBtnClasses = isSolid
        ...
        : "text-white/90 hover:text-white border-white/40";

    // ANA MENÜ YAPISI (İngilizce)
    const navStructure = [
    ...
    return (
        ...
        >
            <div className="px-4 md:px-10 lg:px-20 flex justify-center relative">
                <div className="flex w-full max-w-[1400px] items-center justify-between h-16">
                        ...
                        {logoUrl ? (
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <div className={`w-8 h-8 ${isSolid ? "text-blue-600" : "text-white"}`}>
                                    <svg xmlns="[http://www.w3.org/2000/svg](http://www.w3.org/2000/svg)" viewBox="0 0 24 24" fill="currentColor" className="w-full h-full">
                    ...
                    {/* DESKTOP MENU */}
                    <div className="hidden lg:flex items-center gap-6 xl:gap-8 h-full ml-auto">
                        {navStructure.map((item) => (
                            ...
                            >
                                <Link
                                    href={item.href}
                                    className={`${linkClasses} relative group flex items-center gap-1`}
                                >
                                    {item.title}
                                    {/* SVG Chevron Down */}
                                    {item.slug && (
                                        <svg xmlns="[http://www.w3.org/2000/svg](http://www.w3.org/2000/svg)" viewBox="0 0 20 20" fill="currentColor" className="w-4 h-4 opacity-70 group-hover:rotate-180 transition-transform duration-300">
                    ...
                    {/* RIGHT ACTIONS (LOGIN BUTONU EKLENDİ) */}
                    <div className="hidden lg:flex items-center gap-4 ml-8 border-l border-gray-200/20 pl-8">
                        ...
                        <Link
                            ...
                            className={`flex items-center gap-2 px-3 py-1.5 rounded-full border transition-all text-sm font-semibold uppercase tracking-wider ${loginBtnClasses}`}
                        >
                    ...
                    {/* MOBİL MENÜ BUTONU (EN SAĞDA) */}
                    <div className="lg:hidden flex items-center z-50 ml-4">
                        <button
                            onClick={() => setIsOpen(!isOpen)}
                            className={`p-2 rounded-lg transition-colors ${isSolid ? "text-gray-900" : "text-white"}`}
                        >
                            {isOpen ? (
                            ...
                            <div className="grid grid-cols-4 gap-8 relative z-10">
                                <div className="col-span-1 border-r border-gray-100 pr-6">
                                    ...
                                    <p className="text-sm text-gray-500 leading-relaxed">
                                        Explore our {navStructure.find(n => n.slug === hoveredMenu)?.title.toLowerCase()} opportunities and resources.
                                    </p>
                                </div>
                    ...
                    <div className="px-4 py-6 space-y-2">
                        {navStructure.map((item) => (
                            <div key={item.title}>
                                <Link
                                    ...
                                    className="flex justify-between items-center px-4 py-3 text-base font-medium text-gray-700 hover:text-blue-600 hover:bg-blue-50 rounded-lg transition-colors"
                                >
                                    {item.title}
                                </Link>
                        ...
                        <div className="pt-4 mt-4 border-t border-gray-100">
                            <Link
                                href="/login"
                                onClick={() => setIsOpen(false)}
                                className="flex items-center justify-center gap-2 w-full px-4 py-3 text-center bg-gray-100 text-gray-800 font-bold rounded-lg border border-gray-200"
                            >
                                <svg xmlns="[http://www.w3.org/2000/svg](http://www.w3.org/2000/svg)" viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5">
                                    <path fillRule="evenodd" d="M7.5 6a4.5 4.5 0 119 0 4.5 4.5 0 01-9 0zM3.751 20.105a8.25 8.25 0 0116.498 0 .75.75 0 01-.437.695A18.683 18.683 0 0112 22.5c-2.786 0-5.433-.608-7.812-1.7a.75.75 0 01-.437-.695z" clipRule="evenodd" />
                                </svg>
